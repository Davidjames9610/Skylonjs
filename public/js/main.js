...
function updatefuel() {
    ...
    var w1 = Math.round((100 - myeng.getw1())*10)/10;
    $(".LH2fwdlabel-content").html(w1 + "%");
    //debug = w1;

    var w2 = Math.round((100 - myeng.getw2())*10)/10;
    $(".LOXfwdlabel-content").html(w2 + "%");

    var w3 = Math.round((100 - myeng.getw3())*10)/10;
    $(".LOXaftlabel-content").html(w3 + "%");

    var w4 = Math.round((100 - myeng.getw4())*10)/10;
    $(".LH2aftlabel-content").html(w4 + "%");

}

function updatecofg(time) {

    //first update cofg aims according to the current speed

    var currentmach = MachInfoG[time] / 343;

    //top

    if (currentmach <= 1) {
        cofgTop = (6 * currentmach) + 51;
        $(".cofg-aim-top").css("top", translatecofg(cofgTop) + "%");
    } else if (1 < currentmach <= 1.5) {
        cofgTop = currentmach + 56;
        $(".cofg-aim-top").css("top", translatecofg(cofgTop) + "%");
    } else if (1.5 < currentmach) {
        cofgTop = ((1.5 / 5) * currentmach) + 57.05;
        $(".cofg-aim-top").css("top", translatecofg(cofgTop) + "%");
    }
    ...
    } else if (0.6 < currentmach <= 1.33) {
        cofgMid = (5.33 * currentmach) + 49.6677;
        $(".cofg-aim-mid").css("top", translatecofg(cofgMid) + "%");
    } else if (1.33 < currentmach) {
        cofgMid = (0.33 * currentmach) + 56.33;
        $(".cofg-aim-mid").css("top", translatecofg(cofgMid) + "%");
    }
    ...
    } else if (0.5 < currentmach <= 1.5) {
        cofgBot = (5.4 * currentmach) + 47.9;
        $(".cofg-aim-bot").css("top", translatecofg(cofgBot) + "%");
    } else if (1.5 < currentmach) {
        cofgBot = (0.38 * currentmach) + 55.52;
        $(".cofg-aim-bot").css("top", translatecofg(cofgBot) + "%");
    }
        
    // use fuel to set the position of the actual cofg 
    
    var currentcofg = Math.round(myeng.getcofg() * 100) / 100;
...
function updatedebug(time) {
    ...
    $(".debug-velocity").html("The current velocity is: Mach " + Math.round((MachInfoG[time] / 343) * 100) / 100);
    $(".debug-altitude").html("The current altitude is: " + Math.round((HeightInfoG[time]) * 100) / 100 + "m");
